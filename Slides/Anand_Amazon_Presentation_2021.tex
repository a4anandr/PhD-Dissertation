...
\framesubtitle{Stochastic optimal control}
 \only<1-4>{\alertb{Example:} In average cost optimal control problems,
\[
c(x,u) + P_u h(x) = h(x) + \eta
\]
\begin{tabular}{rl}
$c(x,u)$ & - Cost function associated with state $x$ and action $u$.\\ \pause
$P_u$ & - Transition kernel of the controlled Markov chain,\\ \pause
\alt<3>{& i.e. $P_u h(x) \eqdef \Expect[h(X_{t+1})|X_t = x, U_t  = u]$.\\ \pause}{} 	
$\eta$ & - Average cost.\\ 
{\LARGE \alertb{$h$}} & - {\LARGE \alertb{Relative value function}} \\ \pause
\end{tabular}\\[1em]
...
\only<6->{
	...
	\begin{tabular}{rl}
		$c(x,u)$ & - Cost function associated with state $x$ and action $u$.\\
		$P_u$ & - Transition kernel of the controlled Markov chain. \\	
		$\discount$ & - Discount rate,  $\discount \geq 0$ (usual notation $\beta = \frac{1}{1+\discount},\, \beta \leq 1$).\\
