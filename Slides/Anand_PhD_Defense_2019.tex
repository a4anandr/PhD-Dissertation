...
\item Impact of gain approximation error on filtering performance. \pause - \rd{\text{\sffamily X}}  \pause \\[0.5cm]
\bl{New direction} \\
...
\begin{minipage}[t][6.5cm][t]{\textwidth}
	...
	\only<1-2>{\alertb{Discounted-cost value function:}
		\[
		h^\discount(x) := \int_0^\infty \exp(-\discount t )\Expect_x[c(\markovstate_t)] \ud t,  \qquad \discount >0 \text{ - discount factor} 
		\]\\[-0.75em]
	...
	\only<3>{
	 For a linear parameterization\\[-0.5em]
		\[
		\begin{aligned}
	...
	\only<5>{
		...
		\[
        \begin{aligned}
		 \text{Resolvent kernel - } \small R_\discount c\, (x) & \eqdef\int_0^\infty \Expect_x\Bigl[ \exp \, (-\discount t) c(\markovstate_t)\Bigr] \ud t \\
...
\only<7>{ 
 ...
 \[
\begin{aligned}
 \text{Eligibility vector - } \eligibvector_\basis(r) &\eqdef \int_0^\infty \exp(-\discount (t -r)) \basis(\markovstate_{r-t}) \ud t\\
...
\only<8>{
	...
	& = \Expect[\eligibvector_{\basis_i}(0) c(\markovstate_0)] 
	\end{aligned}
	\]
} 
...
\]
Drawback: Requires the existence of  a regenerating state.
\end{minipage}
\end{frame}
...
Idea: Approximate the gradient of $h$ directly \bl{[RDM 16, DM 16]}:
% \Fontvib
\[
...
\[
\begin{aligned}
\text{where,} \qquad M &=  (1+ \lambda_{mem})\sum_{k=1}^d M_{x_k}^\transpose M_{x_k} + \lambda N M_{00}
\\
b &=  M_{00} \vectilc  + \lambda_{mem} \sum_{k =1}^d  M_{x_k}^\transpose \kFPF_{n-1,k}
\end{aligned}
\]
}
\only<4->{
Both refinements 1 and 2 can be applied independently or simultaneously.
}
\end{overlayarea}
\end{frame}
...
\begin{minipage}[t][6.5cm][t]{\textwidth}
	\begin{tabular}{lll}\alertb{Example:} & For a fixed $t$, $\pr_t$ a Gaussian mixture
		...
		&   $c(x)\equiv x$
		\\
		& $T  = 10^4, 10^5, 10^6$ with $\delta = 0.01$
		% \\
	\end{tabular}
	%\\[-0.3em]
