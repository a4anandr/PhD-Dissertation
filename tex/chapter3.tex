...
The minimizer $f_\reg^* \in \clH$ of the regularized risk \eqref{e:erm_rkhs} for an arbitrary loss function $L(y^i, f(x^i))$ lies in the span of kernel functions centered at the sample points, $\Kern(x^i, \cdot)$,
\[
...
\begin{theorem}[Extended Representer Theorem]
	...
	Then the  optimizer $g^*$ of \eqref{e:erm_rkhs_gradient} over $g\in\clH$ exists, is unique, and has the following representation: \anand{change $x^i_k$ to $x^k$}
	\begin{equation}
	g^*(x) = \sum_{i=1}^N  \Bigl[
	\rkhsparam_i^{0*}  K(x^i,x)   +  \sum_{k =1}^d  \rkhsparam_i^{k*} \frac{\partial K}{\partial x^i_k} (x^i, x) \Bigr],
	\label{e:ext_rep_theorem}
	\end{equation}
...
\begin{proof}
 ...
 \end{lemma}
 By \Lemma{lemma:der_rep_property}, $\frac{\partial K}{\partial x^i_k}(x^i,.) \in \clH$ for each $k$.
