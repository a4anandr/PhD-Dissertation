...
\subsection{Choices of the kernel function}
...
%\[
%\clH_\parallel := \Bigl\{f \in \clH\, |\, f = \sum_{i=1}^N  \beta_i \Kern(x_i,\cdot) \Bigr\},
%\]
%where $N$ ranges over $\mathbb{Z}_+$, and  each $\beta_i \in \Re$ for each $i$. Denote by $\clH_{\perp}$, the subspace of $\clH$ orthogonal to $\clH_\parallel$:
%\[
...
\end{equation}
where gradient terms of the estimator $g$ are included in the function $L$, it cannot be applied directly. The extension to include differential loss (including gradient terms) appeared in \cite{zho08}.    
...
\begin{theorem}[Extended Representer Theorem]
	...
	\[
	g^*(x) = \sum_{i=1}^N  \Bigl[
	\beta_i^{0*}  K(x_i,x)   +  \sum_{k =1}^d  \beta_i^{k*} \frac{\partial K}{\partial x_k} (x_i, x) \Bigr],
	\]
	where $\{\beta_i^{k*} \colon i=1,\cdots,N,\, k = 0,\cdots,d\}$ are real numbers.
	\qed
\end{theorem}
\begin{proof}
 The proof provided here is a special case of the proof for a more general version of the extended representer theorem appearing in \cite{zho08}. The partial derivative reproducing property of the kernel $\Kern$ is stated and proved in \cite[Theorem 1]{zho08}, presented as \Lemma{lemma:der_rep_property}.  
 \begin{lemma}
 ...
 \begin{equation}
 \frac{\partial f(x)}{\partial x_k} = \langle \frac{\partial K(x, \cdot)}{\partial x_k} , f(\cdot) \rangle_\clH \qquad \forall x \in \state, \forall f \in \clH.
 \end{equation}
 \end{romannum}
...
Let us denote by $\clH_\parallel$, the linear subspace of $\clH$ made up of all such functions $f$,
	\[
	\clH_\parallel := \Bigl\{f \in \clH\, |\, f = \sum_{i=1}^N \Bigl[ \beta^0_i K(x_i,.) + \sum_{k=1}^d \beta_i^k \frac{\partial }{\partial x_k} K(x_i, .)\Bigr]\Bigr\},
	\]
	where $N$ ranges over $\mathbb{Z}_+$, and  each $\beta_i^k \in \Re$ for each $i$ and $k = 0$ to $d$. Let $\clH_{\perp}$ be the subspace of $\clH$ orthogonal to $\clH_\parallel$:
	\[
	\clH_\perp : = \{ \tilf \in \clH\,| \, \langle \tilf , f \rangle_\clH = 0 ,\, \forall f \in \clH_\parallel \}
	\]
	...
	&  = \langle g_\parallel\, , \frac{\partial K}{\partial x_k} (x_i,.) \rangle_\clH + \langle g_\perp \, ,\frac{\partial K}{\partial x_k} (x_i,.) \rangle_\clH \\
	&  = \langle g_\parallel\, , \frac{\partial K}{\partial x_k} (x_i,.) \rangle_\clH \\
	&  = \frac{\partial g_\parallel(x_i) }{\partial x_k} \qquad \forall k.
	\end{aligned}
	\]
	Therefore,
	\[
	...
	\]
	The empirical error term depends only on the component $g_\parallel$. Hence, the regularized objective function is minimized if $g^*$ lies within $\clH_\parallel$ and takes the form,
	\[
