...
\end{equation}
for $f$ in some domain. If $f \in C^2$ then, subject to growth conditions \cite{ethkur86},
\begin{equation}
...
\eligibvector_g(r) \eqdef \int_0^\infty \exp(-\discount r) g(\markovstate_{t-r}) \ud r,
...
& = \Expect_{\markovstate \sim \pr}\Bigl[f(\markovstate_0) \underbrace{\int_0^\infty \exp(-\discount t) g(\markovstate_{-t})\ud t}_{\eligibvector_g(0)}\Bigr]\\
...
This leads to an interesting interpretation of the exponential term $\exp(-\clU_r^t)$ as implicitly introducing a ``discounting factor'' to the average-cost problem, under strong convexity of $U$. 
...
\end{equation}
More recently, a similar two-time-scale stochastic approximation algorithm ensures the stability of the algorithm. Interested readers are referred to \cite{devmey17a}. \anand{thesis}
%\rd
