...
In this section, we introduce the Langevin diffusion and its associated Poisson's equation with only the requisite amount of technical detail, so that the broad motivation is made clear. A more elaborate discussion can be found in \Section{s:mcmc_langevin} in the context of MCMC algorithms. 
...
\end{equation}
for $f$ in some domain. If $f \in C^2$ then, subject to growth conditions in \cite{ethkur86},
\begin{equation}
\generate f = -\nabla \pot \cdot \nabla f + \Delta f,\qquad f\in C^2,
\label{e:diff_td_langevin_generator}
\end{equation}
where $\nabla$ denotes the gradient and $\Delta$ is the Laplacian. Under conditions on $U$, the SDE \eqref{e:diff_td_langevin_cts} defines a strong Markov semigroup $\{P_t\}_{t \geq 0}$. The generator $\generate$ can be written in terms of the semigroup as,
\begin{equation}
...
\end{equation}
The notation $\Expect_x[\tilc(\markovstate_t)]$ is shorthand for the conditional expectation $\Expect[\tilc(\markovstate_t)|\markovstate_0 = x]$, with $x$ as the initial state. In average-cost optimal control, where $c$ denotes a one-step cost function, the function $h(x)$ is called the relative value function and can be interpreted as the infinite-horizon expected normalized cost with the initial state $\markovstate_0 = x$. .  
...
\end{equation}
It may be noted that the Langevin diffusion \eqref{e:diff_td_langevin_cts} is a special case of \eqref{e:diff_td_cts_diffusion} with $a(x) = -\grad U(x)$ and $\sigma(x) \equiv \sqrt{2}$, and the associated differential generator is defined in \eqref{e:diff_td_langevin_generator}. 

Consider a parameterized family of approximations $\{h^\param : \param \in \Re^\ell\}$. In the case of a linear parameterization, where we have $\ell$ functions on $\Re$ as the basis, denoted $\{\basis_i : 0 \leq i \leq \ell\}$, the parameterized family $\{h^\param\}$ becomes, 
\begin{equation}
h^\param(x) := \sum_{i=1}^\ell \param_i \basis_i(x) = \param^\transpose \basis(x), \qquad x \in \Re
...
\end{aligned}
\end{equation}
Among the conventional TD algorithms, only TD($1$) has an interpretation as a norm minimization problem. It is evident from the definition of $\error(\param)$ that it penalizes the approximation error more strongly for states with larger invariant probability $\pr$. As noted in \cite{ctcn}, these are the states that are visited more often and hence, a better approximation of $h^\discount$ is desired at these points on $\state$. A more important benefit of using the $L^2(\pr)$ norm is that it allows the construction of an algorithm using Monte Carlo methods. 
...
%\end{aligned}
% \end{equation}
The following transformation can be applied to $b$, using \eqref{e:diff_td_lstd_hgamma_resolvent} followed by an adjoint trick. Let $\bfPhi=\{\markovstate_t : t\in\Re\}$ denote a stationary version of a continuous-time diffusion process and define,
\begin{equation}
\begin{aligned}
b_i = \Expect [h^\discount(\markovstate_t) \basis_i(\markovstate_t) ] &= \langle h^\discount, \, \basis_i \rangle_{L^2}\\
& = \langle R_\discount c, \,\basis_i \rangle_{L^2} \\
& = \langle c,\, R^\dagger_\discount \basis_i \rangle_{L^2},
...
For real-valued measurable functions $f,g$ with at most exponential growth we have,
\begin{equation}
\langle R_\discount f, \, g \rangle_{L^2}   =  \langle f, \, R_\discount^\dagger g \rangle_{L^2} =  \Expect [ f(\markovstate_t)	\eligibvector_g(t)   ]\,, \quad t\in\Re,
...
\eligibvector_g(r) \eqdef \int_0^\infty \exp(-\discount t) g(\markovstate_{r-t}) \ud t,
...
\begin{equation}
\Expect [f(\markovstate_t) g(\markovstate_0)] = \Expect [f(\markovstate_0) g(\markovstate_{-t})].
\end{equation} 
...
& = \int_0^\infty \exp(-\discount t) \Expect [f(\markovstate_t) g(\markovstate_0)]\ud t \\
& = \int_0^\infty \exp(-\discount t) \Expect [f(\markovstate_0) g(\markovstate_{-t})] \ud t \qquad \text{(Applying the stationarity property of $\markovstate$)}\\
& = \Expect \Bigl[f(\markovstate_0) \underbrace{\int_0^\infty \exp(-\discount t) g(\markovstate_{-t})\ud t}_{\eligibvector_g(0)}\Bigr]\\
& \text{(Applying Fubini's theorem and absolute integrability)}\\
...
We drop the subscript: $\eligibvector_i = \eligibvector_{\basis_i} $ 
\begin{equation}
\eligibvector_i(r) \eqdef \int_0^\infty \exp(-\discount t) \basis_i(\markovstate_{r-t}) \ud t,
\label{e:diff_td_lstd_eligib}
\end{equation}
$b$ in \eqref{e:diff_td_lstd_b_R_adjoint} can be written as,
\begin{equation}
b = \Expect[c(\markovstate_0) \eligibvector(0)].
\label{e:diff_td_lstd_b}
\end{equation}
% \ section{Galerkin approximation}
The function $\eligibvector$ is called the eligibility vector in TD learning. A more general proof using the generalized resolvent kernel $R_G$ appears in the derivation of $\gradTD$ learning in \Section{s:diff_td_learning}.
...
\\
b & \approx  \frac{1}{T} \int_0^T c(\markovstate_t) \eligibvector(t) \ud t
...
\ddt b(t) &=  c(\markovstate_t)\eligibvector(t)
...
\end{proposition} 
\begin{proof}[Proof]
	We can assume that $f$ and $g$ have compact support. The extension to arbitrary functions satisfying the assumptions of the proposition is obtained by approximation in $C^2 \cap L^2(\pr)$. In the following, $\partial_k f$ is used as shorthand for $\frac{\partial f}{\partial x_k}$. 
	...
	&= -\langle f, \generate g \rangle_{L^2}
	\end{aligned}
	\end{equation}
	...
	\end{equation}
\end{proof}
The equality on the right side is a result of the self-adjoint property of the Langevin generator $\generate$. This is valid under the reversibility property of the diffusion.
...
\begin{lemma}
	...
	\end{equation}
	and $k = \| \nabla h \|^2_{L^2}$.  Consequently, assuming $M$ is invertible, the optimizer \eqref{e:diff_td_gradTD_norm_error}
	is:
	\begin{equation}
	\param^* = M^{-1}b.
...
Using \Prop{prop:lang_generator_grad} and Poisson's equation \eqref{e:diff_td_poissons}, $b_i$ in \eqref{e:diff_td_gradTD_M_b} has an alternate representation in terms of known functions:
\begin{equation}
\begin{aligned}
...
Here, the presentation of the algorithm is restricted to the scalar case, where $\state = \Re$. Extensions to dimension $d>1$ is an open topic of research. 
...
Similar arguments are used in the derivation of the LSTD algorithm in \Section{s:lstd}. A special case of \Lemma{lemma:RU_adjoint} with $U''$ replaced with $\discount$ is used in its derivation.  
This leads to an interesting interpretation of the exponential term $\exp(-\clU_r^t)$ as implicitly introducing a ``discounting factor'' to the average-cost problem. If $U$ satifies the conditions in \Prop{prop:regularity_U}, the algorithm takes a form similar to LSTD for the discounted-cost case.  This discounting eliminates the need for a regenerating state for $\markovstate$.
...
\end{equation}
A common choice is $\sagain_t = 1/(t+1)$. By using a matrix gain $M_t$, inspired by Newton-Raphson methods, convergence of the algorithm can be improved. The matrix gain $M^* := -\nabla^2_{\param}\mathcal{E}|_{\param = \param^*}$ is in some cases, optimal in terms of rate of convergence of the algorithm \cite{devmey17a}. In some cases the approximation,
\begin{equation}
M* \approx - \langle \basis'_{\param^*}, \basis_{\param^*}^{'\transpose} \rangle_{L^2}, 
\end{equation}
is justified, leading to the Monte Carlo approximation:
\begin{equation}
M_T  \eqdef  -\frac{1}{T} \int_0^T \basis'_{\param_t}(\markovstate_t) \basis^{'\transpose}_{\param_t}(\markovstate_t) dt
\end{equation}
More recently, a similar two-time-scale stochastic approximation algorithm ensures the stability of the algorithm. Interested readers are referred to \cite{devmey17a}.
%\rd
...
\begin{romannum}
	...
	\item To address the limitations of LSTD, a new class of algorithms called differential LSTD learning that directly approximates the gradient of the solution is introduced.  For the special case of Poisson's equation associated to the Langevin diffusion, adjoint property of the differential generator allows a simple formulation that does not require a simulation of the model.
