...
\end{equation} 
	\anandspm{Conditions under which the exchange of derivative and expectation is valid for a nonlinear parameterization}
...
\begin{algorithm}{$\gradTD$-L algorithm}
	...
	\end{algorithmic}
\end{algorithm} \anand{Probably, this needs to go in Chapter 4, Will come back to this later}
...
The presentation of the algorithm here is restricted to the scalar case, where $\state = \Re$. However, it is expected that the algorithm can be extended to higher dimensions. \anandspm{Should I say anything more?}
...
\end{aligned}
\end{equation} \anandspm{is the sign and notation for $M$ correct?}
and approximated as,
\begin{equation}
