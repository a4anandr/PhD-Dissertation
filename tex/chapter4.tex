...
\label{s:fpf}
In this section, we introduce the feedback particle filter (FPF), which is the preferred approximation of the nonlinear filter in this dissertation. In \Section{s:approx_nl_filter}, several approximations to the nonlinear filter, including the EKF and the particle filter were presented. \rd{In addition to providing an overview of such techniques, one of the purposes behind giving a detailed exposition was to enable us to compare and contrast the similarities (and dissimilarities) of these techniques with the FPF.}  
...
\subsection{Galerkin-based Methods} \anandspm{Can you quickly glance through this section?}
\label{s:galerkin}
...
\]
where $\{e_j\}$ are the standard basis elements in $\Re^d$. This approximation has been successfully tested in \cite{tilghiomeh13}. The constant gain is an important component of the refinements to the $\gradTD$-RKHS algorithm proposed in \Section{s:fpf_rkhs_improvements}.
...
\begin{algorithm}{$\gradTD$-L algorithm with finite basis}
	\begin{algorithmic}[1]
...
\begin{algorithm}{$\gradTD$-RKHS algorithms for gain function approximation}
	\begin{algorithmic}[1]
		...
		% \label{alg:rkhs}
	\end{algorithmic}
\end{algorithm} \anandspm{Could you check the $\gradTD$-RKHS algorithm description?}
...
\begin{figure}[htbp]
	...
	\caption[$\gradTD$ performance comparison with finite basis]{Gain function approximations using $\gradTD$ learning algorithm (\Section{s:diff_td_learning}) using a linear parameterization \cite{raddevmey16} A) Polynomial basis, B) Fourier basis consisting of sines and cosine functions, C) Polynomials weighted by $\pr_i$.}
	\label{fig:diff_td_linear}
\end{figure}

\begin{figure}
	\includegraphics[width=6in]{images/Chap4_diff_td_nl}
	\caption[$\gradTD$ performance with nonlinear parameterization]{Gain function approximations using a $9$-dimensional nonlinear parameterization of the form using $\gradTD$-L algorithm (\Section{s:diff_td_learning})}
	\label{fig:diff_td_nl}
\end{figure}

\begin{figure}
	\includegraphics[width=6in]{images/Chap4_lang_td_wt_polynomials}
	\caption[$\gradTD$-L performance with polynomial basis]{Gain function approximations using a $10$-dimensional polynomial$\times \pr_i$ basis using $\gradTD$-L algorithm (\Section{s:diff_td_langevin})}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width = 6in]{images/Chap4_param_est_state_comparison}
