...
In this section, we present a number of numerical experiments to compare the performance of the various gain approximation algorithms discussed in this dissertation. The list of experiments can be broadly classified as:
\begin{itemize}
 \item Those concerning approximating the gain for a fixed time $t$ 
 \item Those concerning an online filtering problem. 
\end{itemize}
The second set of experiments is more challenging as it requires continuous estimation of the gain at the sampling instants. 
%In \Section{s:fpf_em}, 
...
\begin{arabnum}
	...
	\item  Obtain an approximation within $\clK$ for the FPF gain using $\gradTD$ learning algorithm (\Section{s:diff_td_learning}), based on the solution to Step~1.
\end{arabnum}
...
\end{equation}
\noindent
where each $\pr_{i}$ is a Gaussian density with mean $\mu_{i}$ and standard deviation $\sigma_{i}$, and $w_i$ as its weight. In this case, the vector $\alpha_i \eqdef [ \mu_i, \sigma_i, w_i]$. 
...
Kernel density estimators (KDE) offer a non-parametric alternative to the EM algorithm. This has not been explored. The choice of $\clK$ is also problem-specific -- examples are given in \Section{s:linear_nl}. The EM algorithm followed by the simulation of the Langevin SDE for implementing the $\gradTD$ learning algorithm adds a layer additional complexity to the FPF. For an online estimation, the EM algorithm can be initialized with the ML parameters obtained from the previous instant for faster convergence.   

\subsection{Numerical issues with the gain}
In this section, we highlight the numerical issues associated with the optimal FPF gain and how the approximation method described in \Section{s:fpf_em} can be adapted to ensure the stability of the filter. For $d=1$, the optimal gain takes the integral form in \eqref{e:fpf_gain_1d}:
\[
\kFPF(x) = - \frac{1}{\pr(x)} \int_{-\infty}^{x} (c(y) - \hat{c}) \pr(y) \ud y
\]
If $c(x) = x$ and $\pr$ a Gaussian mixture model of the form \eqref{e:fpf_gaussian_mix}, the gain $\kFPF(x)$ is given by,
\begin{equation}
...
One of the practical approaches to prevent $\kFPF$ from taking extremely high values is to introduce a third Gaussian density $\pr_0$ to ``fill the well''. The third density $\pr_0(x)$ is chosen as $\mathcal{N}(\mu_{0}, \sigma_{0}^{2})$, where $\mu_0$ and $\sigma_0$ correspond to the sample mean and standard deviation of the entire particle population. The mixture probability $w_0$ of $\pr_0$ in the overall distribution $\pr$ is forced to be greater than a threshold value. This is further illustrated in the peak magnitude of $\kFPF_1$ in \Fig{fig:fpf_gain_num_issues}.
...
\]
such that each component has a variance of $0.2$. 

\subsubsection*{Linear parameterization}
\label{s:linear_param}
It is reasonable to search for a basis that offers flexibility where the density $\pr$ takes on non-negligible values. We consider three different choices for basis functions as follows:
\begin{romannum}
...
\item Polynomials weighted by the component densities $\pr_i$. 
\[
\basis_n(x) \eqdef x^n \pr_1(x), \qquad 
\basis_{n+1}(x) \eqdef x^n \pr_2(x),
\] 
for $1 \leq n \leq \ell/2$. 
\end{romannum}
It is not difficult to show that the true gain is nearly constant for large $x$, with $\lim_{|x|\to\infty} \kFPF(x) = \Kkal$. The limit corresponds to the Kalman-like gain $\Kkal$ obtained for the model in which $\pr$ is replaced by $\pr_i$  (the Gaussian density with the highest variance). The function $\basis_n(x) = x$ is added to the basis (for cases ii and iii, it is already included in i), such that $\nabla \basis(x)\equiv 1$ is present to account for this constant asymptotic gain value. The class $\clK$ is defined using this basis:
\begin{equation*}
...
\begin{figure}[htbp]
	...
	\label{fig:lstd_linear}
\end{figure}

%\begin{figure}[h]
%	\includegraphics[width=7in]{images/Chap4_diff_td_linear_polynomials}
%	\caption{$\nabla$-LSTD learning for the linear parameterization.}
%	\label{fig:polynomial}
%\end{figure}
%\begin{figure}[h]
%	\includegraphics[width=7in]{images/Chap4_diff_td_linear_fourier}
%	\caption{$\nabla$-LSTD learning for the linear parameterization.}
%	\label{fig:fourier}
%\end{figure}
%\begin{figure}[h]
%	\includegraphics[width=7in]{images/Chap4_diff_td_linear_wt_polynomials}
%	\caption{$\nabla$-LSTD learning for the linear parameterization.}
%	\label{fig:wt_polynomial}
%\end{figure}
...
\caption[RKHS and coifman performance]{Gain function approximations using a $10$-dimensional polynomial$\times \pr_i$ basis using $\gradTD$ learning for Langevin diffusion (\Section{s:diff_td_lang})}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\mbox{
		\subfigure [] {\includegraphics[width=6in]{images/Chap4_N_dN}} 
	}
	\mbox{
...
\Fig{fig:lstd_linear} compares the performance of the various choices of basis functions for different simulation times $T = 10^4, 10^5, 10^6$ for the Langevin SDE. An Euler-discretization scheme was implemented with $\delta = 0.01$. The true FPF gain computed using the integral formula \eqref{e:fpf_gain_1d} is also plotted for comparison. with the best approximation from within the parameterized family obtained using $\nabla$-LSTD learning. Although, the approximation is not tight everywhere. As expected, the accuracy is typically better at values of $x$ for which $\pr(x)$ is large.
Weighted polynomials is better because it ensures the gain decays at large $x$ values. 
...
The gain function $K(\oscState,t)$ at each instant $t$ is obtained as the solution to \eqref{e:diff_td_poissons}:
\begin{equation*}
- U'(\oscState) K(\oscState) + K'(\oscState) = -\tilc(\oscState)  %\qquad \{\tilc =c-\int c(\oscState)\pr(\oscState,t)d\oscState\}
\end{equation*}
...
was
taken to be a Gaussian mixture given by $1/2 \mathcal{N}(-1,0.4) + 1/2 \mathcal{N}(1,0.4)$. It is shown as the shaded region in \Fig{f:gain_comparison}. The number of particles was fixed at $N = 500$. The particles $\{x^i_0\}_1^N$ were drawn i.i.d.\ according to $\pr_0$;  the same seeds were used in each FPF implementation.

The following approximation schemes were compared: finite dimensional approximation, the RKHS-OM method with and without memory, the Markov semigroup approximation method of \cite{tagmeh16a}, the constant gain approximation \eqref{e:fpf_gain_const_approx}, and the exact gain   \eqref{e:fpf_kfpf1d}.  For finite dimensional approximation, we chose $20$ basis functions:  each basis function was of the form $x^k \varrho^i(x)$ in which $1\le k\le 10$,  and $\rho^i$ is a Gaussian pdf centered at mode $i$, for $i=1,2$.   
...
\textbf{Sensitivity to $\lambda$ and $\epsy$} For each of the RKHS methods, the Gaussian kernel \eqref{e:fpf_gauss_kernel} was used, centered at the particle locations $\{x^i\}_1^N$. \Fig{fig:log_mse_contour} illustrates the sensitivity in gain approximation
to the parameters
