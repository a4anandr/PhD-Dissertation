...
The main difference in \eqref{e:fpf_emp_posterior} compared to the estimate in \eqref{e:particle_empirical} is that all the particles are weighted equally in the FPF. Similar unweighted particle filter approaches were proposed in \cite{mitnew04,crixio05}. Daum et al. discuss the limitations of the conventional particle filter and present an unweighted filtering scheme for the continuous-discrete time case, called the information flow filter in. There are similar Monte-Carlo implementations of the nonlinear filter in the literature, such as   Daum's particle flow filter \cite{dauhua10}. See \cite{taoyang_acc14} for a survey. In this dissertation, the discussion is restricted to the scalar filtering problem. However, all of the theory is valid for multidimensional problems as well.
...
\label{e:fpf_el_bvp}
\end{equation}
where $c'(x) = \frac{\partial c }{\partial x}$. Denoting $\pot=-\log(\pr)\in C^1$ and assuming its gradient is globally Lipschitz continuous, it can be shown that the EL-BVP in \eqref{e:fpf_el_bvp} takes the standard form of Poisson's equation for the Langevin diffusion \eqref{e:poissons}.
\begin{equation}
-\pot' K + K''  = -\tilc, \qquad \tilc = c - \hat{c},
\end{equation}
The formula for the gain is obtained through the following steps: Obtain a solution $h$ to Poisson's equation:
\begin{equation}
\generate h = -\tilc \,
\label{e:fpf_gain_poissons}
\end{equation}
where $\hac = \int c(x) \pr(x)\, \ud x$.  Conditions on $U$ and $c$ under which the solution $h$ and its gradient exists has been discussed in \Chapter{ch:diff_td}. Under these conditions, the gain coincides with the gradient:
\begin{equation}
\kFPF (x) = \nabla h(x)\, ,  \quad x\in\state\, .
\label{e:fpf_k_gradient}
\end{equation}

When $\pot$ is quadratic and $c$ is linear,  then $\kFPF$ is a constant independent of $x$, and can be interpreted as a Kalman gain \cite{yanlaumehmey13}. In the general scalar case $d=1$, $\kFPF$ has an explicit solution:
\begin{equation}
\kFPF(x) = - \frac{1}{\pr(x)} \int_{-\infty}^{x} (c(y) - \hat{c}) \pr(y) \ud y.
\label{e:kfpf1d}
\end{equation}
The exact computation of $\kFPF$ is intractable for $d > 1$ outside of particular cases. This motivates approximation techniques to obtain an estimate of $\kFPF$ that is optimal in some meaningful metric.
...
\begin{aligned}
\beta ^*_n =
\argmin_{\beta  \in \Re^N}  &  \frac{1}{N} \Bigl[ (1 + \lambda_1) \beta ^\transpose \Bigl( \sum_{k=1}^d M_{x_k}^\transpose M_{x_k} \Bigr) \beta  \\
& -  \beta ^\transpose  ( 2 \lambda_1 \sum_{k=1}^d M_{x_k}^\transpose \kFPF_{n-1,k} +M_0 \vectilc  \Bigr) ] + \lambda \beta ^\transpose M_0 \beta
\end{aligned}
\end{equation}
