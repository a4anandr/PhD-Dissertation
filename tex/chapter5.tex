...
In recent research, it has been found that under certain assumptions, the asymptotic variances of certain well known MCMC algorithms like the Unadjusted Langevin Algorithm (ULA) and Random Walk Metropolis (RWM) are close to the asymptotic variance of the Langevin diffusion \cite{brodurmeymourad18}. Hence, the application of the same control variate techniques is justified.  Applications to numerical examples involving logistic regression (logit) and probit models are discussed in \Section{s:mcmc_numerics}. 
...
\end{equation} %\anandspm{Is there a better notation ${\asymvar^\param}^2$?}
